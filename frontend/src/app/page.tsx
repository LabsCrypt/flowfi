...
export default function StreamDetailsPage() {
  ...
  const [pauseResumeStatus, setPauseResumeStatus] = useState<TransactionStatus>("idle");
  const [pauseResumeTxHash, setPauseResumeTxHash] = useState<string | undefined>(undefined);
  const [pauseResumeError, setPauseResumeError] = useState<string | undefined>(undefined);
  ...
  const handleTopUp = async () => {
    ...
    try {
      await topUpStream(session, {
        ...
        amount: BigInt(parseFloat(topUpAmount) * 1e7),
      });
  ...
  const canWithdrawStream = isConnectedRecipient && displayedClaimable > 0;
  ...
  return (
    <main style={{ minHeight: "100vh", padding: "clamp(1rem, 3vw, 2rem)" }}>
      <div style={{ maxWidth: 720, margin: "0 auto", display: "grid", gap: "1rem" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <p style={{ margin: "0.2rem 0", color: "var(--text-muted)" }}>
                Sender:{" "}
                <code style={{ background: "rgba(19,38,61,0.07)", borderRadius: "0.4rem", padding: "0.15rem 0.45rem", fontSize: "0.85rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              ...
              <div>
                ...
                <p style={{ margin: "0.2rem 0 0", fontSize: "0.9rem", color: "var(--text-muted)" }}>
                  {stream.pausedAt
                    ? `Paused at ${new Date(parseInt(stream.pausedAt) * 1000).toLocaleString()}`
        ...
        <div className="dashboard-panel">
          ...
          <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
            {canWithdrawStream && (
              <Button onClick={() => void handleWithdraw()} disabled={withdrawing || withdrawStatus === "submitted"} glow>
                {withdrawing ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Withdrawing...</> : "Withdraw"}
              </Button>
            )}
            <Button onClick={() => setShowTopUp(!showTopUp)} disabled={!stream.isActive} variant="outline">
              {showTopUp ? "Cancel Top-Up" : "Top Up"}
            </Button>
            {canCancelStream && (
              <Button onClick={() => setShowCancelModal(true)} disabled={cancelling} style={{ borderColor: "#ef4444", color: "#ef4444" }} variant="outline">
                Cancel Stream
              </Button>
            )}
            {stream.isActive && !stream.isPaused && session?.publicKey === stream.sender && (
              <Button onClick={handlePause} disabled={pausing} style={{ borderColor: "#f59e0b", color: "#f59e0b" }} variant="outline">
                {pausing ? "Pausing..." : "Pause Stream"}
              </Button>
            )}
            {stream.isActive && stream.isPaused && session?.publicKey === stream.sender && (
              <Button onClick={handleResume} disabled={resuming} glow>
          ...
          {showTopUp && (
            <div style={{ marginTop: "1rem", display: "flex", gap: "0.5rem" }}>
              <input type="number" placeholder="Amount" value={topUpAmount} onChange={(e) => setTopUpAmount(e.target.value)} style={{ padding: "0.5rem", borderRadius: "0.25rem", border: "1px solid var(--glass-border)", background: "rgba(255,255,255,0.05)", color: "inherit" }} />
          ...
          {(pauseResumeStatus !== "idle") && (
            <div style={{ marginTop: "1rem" }}>
              <TransactionTracker status={pauseResumeStatus} txHash={pauseResumeTxHash} error={pauseResumeError} onRetry={pauseResumeStatus === "failed" ? (stream.isPaused ? handleResume : handlePause) : undefined} />
            </div>
          )}

          {withdrawStatus !== "idle" && (
            <div style={{ marginTop: "1rem" }}>
              <TransactionTracker status={withdrawStatus} txHash={withdrawTxHash} error={withdrawError} streamId={streamId} onRetry={() => void handleWithdraw()} />
            </div>
          )}
        </div>
      ...
      <CancelStreamModal isOpen={showCancelModal} isCancelling={cancelling} streamId={streamId} onClose={() => setShowCancelModal(false)} onConfirm={() => void handleConfirmCancel()} />
    </main>
  );
}